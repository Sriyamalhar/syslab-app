...
import { 
  Users, Network, Shield, Server, Cpu, 
  Database, Cloud, Zap, Settings, Globe, Activity 
} from 'lucide-react';
  ...
  return (
    <aside className="w-64 border-r border-border bg-card/30 flex flex-col h-full z-10 relative">
      <div className="p-4 border-b border-border bg-card">
        ...
        <p className="text-xs text-muted-foreground">Drag items to canvas</p>
      </div>
              ...
              {expanded[cat.id] && (
                <div className="mt-1 pl-2 space-y-1">
                  {cat.nodes.map(type => (
                    <div 
                      key={type}
                      className="p-2 text-xs border border-border/50 bg-card rounded cursor-grab hover:border-primary/50 transition-colors"
                      draggable
